function plotEnergySankey(out)
% Description: This function represents the energy flow in the vehicle as a
%              Sankey diagram
%-----------------
...
%% Implementation
% Energy and labels of all possible losses:
...
% Draw Sankey graph
if out.energyCharger(end)>0
    plotSankeyVertical([out.energyBatteryStart,out.energyCharger(end)], reqEn, 'kWh', {'Battery','Charged',labels{:},'Rem. Energy'});
else
    plotSankeyVertical([out.energyBatteryStart], reqEn, 'kWh', {'Battery',labels{:},'Rem. Energy'});
end

end

%% Subfunction:
function plotSankeyVertical(inputs, losses, unit, labels)
...
elseif any(losses < 0) || any(inputs < 0)    
    ...
    % Filter out empty text that might have been mistakenly created:
    count = 1;
    for i=1:numel(h)
        if isempty(h(i).String)
            id(count) = i;
            count = count+1;
        end
    end
    ...
    % Consider the dimensions of all lines as well:
    h = findall(gcf,'Type','line');
    allXData = [];
    allYData = [];

    % Loop through each element in the struct h
    for i = 1:length(h)
        % Concatenate the XData from each element
        allXData = [allXData, h(i).XData];
        allYData = [allYData, h(i).YData];
    end

    %set correct axis limits%
    set(gca,'YLim',[min(min(allYData),min(Postext(2,:))-0.1), max(max(allYData),max(Postext(2,:))+0.1)]);
    set(gca,'XLim',[min(min(allXData),min(Postext(1,:))-0.3), max(max(allXData),max(Postext(1,:))+0.6)]);
